...
const user_phantom_container = require("../models/userContainer.model")
const cookie = require('../models/user_cookie.model');
const phantomResponse=require("../models/response.model");
...
//  const user =await User.findOne({$or:[{ date:{$lt:userdate}},{date:null}]}).sort({_id:-1});
  ...
  {
  sdk.auth('MIdlWFYwQRCINIBNaaWZ6QRw4MEvN5wJYDymKMqeC4Q');
  sdk.postAgentsLaunch({ id: agentData.agent_id,argument: {inboxFilter: 'all',sessionCookie: cookieData.cookie_value,before: '05-23-2023'},manualLaunch: true})
  .then(({ data }) => {
...
await sdk.getAgentsFetchOutput({id: agent_id})
.then(({ data }) => {
  let link;
  apifetchdata = data;
  ...
  if(data.status==='finished')
  {
    ...
    return res.send(
      {status : "success",message_status : "Messages have been Inserted",data:data}
    );
...
exports.fetchmessage = async(req,res)=>{
  try
  {
    ...
    {
      var arr = [];
      const team = await User.find({team:req.body.team},{"team":0,"username":0,"email":0,"password":0,"user_type":0,"date":0,"__v":0});
      ...
      for(var i=0;i<team.length;i++)
      {
        ...
        let response = await phantomResponse.find({"user_id":{ $in:arr}}).limit(limit)
        .skip(skip);  
        var count =await phantomResponse.countDocuments({"user_id":{ $in:arr}});
    ...
    else if(req.headers["x-access-token"])
    {
        ...
        var response = await phantomResponse.find({"user_id":decoded.id}).limit(limit)
        .skip(skip);  
    ...
    {
      return res.send({"success":"false","msg":"Please send team or header ","data":[]});
    }
...
async function getUserIdFromUserCotnainer(container_id) {
  try {    
    const container_id_user=  await user_phantom_container.findOne({"container_id":container_id});
